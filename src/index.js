...
console.log("Starting db-packagemanagement-service service...");
...
const start = async () => {
  process.title = "node-db-packagemanagement-service";
  ...
  try {
    ...
    await setRedisData(
      "db-packagemanagement-service:start",
      today.toUTCString(),
    );
    console.log(
      "Service start datetime is written to Redis key db-packagemanagement-service:start",
      today.toUTCString(),
    );
  } catch (err) {
  ...
  console.log(
    "db-packagemanagement-service is listening HTTP/REST port " +
...
/* starting the server db-packagemanagement-service .... */
start();
