...
console.log("Starting db-feedbackmanagement-service service...");
...
const start = async () => {
  process.title = "node-db-feedbackmanagement-service";
  ...
  try {
    ...
    await setRedisData(
      "db-feedbackmanagement-service:start",
      today.toUTCString(),
    );
    console.log(
      "Service start datetime is written to Redis key db-feedbackmanagement-service:start",
      today.toUTCString(),
    );
  } catch (err) {
  ...
  console.log(
    "db-feedbackmanagement-service is listening HTTP/REST port " +
...
/* starting the server db-feedbackmanagement-service .... */
start();
