...
// Publisher class for createFeedbackGuest api
const { FeedbackguestCreatedTopic } = require("./topics");
class FeedbackguestCreatedPublisher extends ServicePublisher {
  constructor(feedbackguest, session, requestId) {
    super(FeedbackguestCreatedTopic, feedbackguest, session, requestId);
  }

  static async Publish(feedbackguest, session, requestId) {
    const _publisher = new FeedbackguestCreatedPublisher(
      feedbackguest,
      session,
      requestId,
    );
