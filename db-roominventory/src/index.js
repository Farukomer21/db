...
console.log("Starting db-roominventory-service service...");
...
const start = async () => {
  process.title = "node-db-roominventory-service";
  ...
  try {
    ...
    await setRedisData("db-roominventory-service:start", today.toUTCString());
    console.log(
      "Service start datetime is written to Redis key db-roominventory-service:start",
      today.toUTCString(),
    );
  } catch (err) {
  ...
  console.log(
    "db-roominventory-service is listening HTTP/REST port " +
...
/* starting the server db-roominventory-service .... */
start();
