...
// Publisher class for listAvailableRooms api
const { AvailableroomsListedTopic } = require("./topics");
class AvailableroomsListedPublisher extends ServicePublisher {
  constructor(availablerooms, session, requestId) {
    super(AvailableroomsListedTopic, availablerooms, session, requestId);
  }

  static async Publish(availablerooms, session, requestId) {
    const _publisher = new AvailableroomsListedPublisher(
      availablerooms,
      session,
      requestId,
    );
