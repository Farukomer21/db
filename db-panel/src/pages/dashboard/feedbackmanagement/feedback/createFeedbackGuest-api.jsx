...
export default function FeedbackManagementCreateFeedbackGuestApiPage() {
  const [view, setView] = useState("Table");
  const [createdFeedbackguest, setCreatedFeedbackguest] = useState(null);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      setError(null);
      setCreatedFeedbackguest(null);
      reset();
      console.info("RESPONSE", response);
      setCreatedFeedbackguest(response.data.feedback);
    } catch (ex) {
      console.error(ex);
      setError(ex);
    }
    ...
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      {(createdFeedbackguest || error) && (
        <Box paddingTop="2rem">
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse content={createdFeedbackguest} error={error} />
            ) : (
              <JsonResponse content={createdFeedbackguest || error} />
            )}
          </Box>
        </Box>
      )}
