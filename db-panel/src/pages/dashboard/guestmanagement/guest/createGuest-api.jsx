...
export default function GuestManagementCreateGuestApiPage() {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      const response = await guestManagementAxios.post(
        guestManagementEndpoints.guest.createGuest,
        data,
      );
      setError(null);
      ...
      setCreatedGuest(response.data.guest);
    } catch (ex) {
      console.error(ex);
      setError(ex);
    }
    ...
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      {(createdGuest || error) && (
        <Box paddingTop="2rem">
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse content={createdGuest} error={error} />
            ) : (
              <JsonResponse content={createdGuest || error} />
            )}
          </Box>
        </Box>
      )}
