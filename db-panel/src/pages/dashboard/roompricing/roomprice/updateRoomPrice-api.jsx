...
export default function RoomPricingUpdateRoomPriceApiPage() {
  const [view, setView] = useState("Table");
  const [updatedRoomprice, setUpdatedRoomprice] = useState(null);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      setError(null);
      setUpdatedRoomprice(null);
      reset();
      console.info("RESPONSE", response);
      setUpdatedRoomprice(response.data.roomPrice);
    } catch (ex) {
      console.error(ex);
      setError(ex);
    }
    ...
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      {(updatedRoomprice || error) && (
        <Box paddingTop="2rem">
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse content={updatedRoomprice} error={error} />
            ) : (
              <JsonResponse content={updatedRoomprice || error} />
            )}
          </Box>
        </Box>
      )}
