...
const ReservationbycodeSchema = zod.object({
...
export default function ReservationManagementCancelReservationByCodeApiPage() {
  const [view, setView] = useState("Table");
  const [updatedReservationbycode, setUpdatedReservationbycode] =
    useState(null);
  ...
  const methods = useForm({
    resolver: zodResolver(ReservationbycodeSchema),
    defaultValues,
  });
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      const response = await reservationManagementAxios.patch(
        reservationManagementEndpoints.reservation.cancelReservationByCode.replace(
          ":reservationId",
          inputReservationId,
        ),
        data,
      ...
      setError(null);
      setUpdatedReservationbycode(null);
      reset();
      console.info("RESPONSE", response);
      setUpdatedReservationbycode(response.data.reservation);
    } catch (ex) {
      console.error(ex);
      setError(ex);
    }
  ...
  return (
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      <Divider />
      {(updatedReservationbycode || error) && (
        <Box paddingTop="2rem">
          <Box
            display="flex"
            flexDirection="row"
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse content={updatedReservationbycode} error={error} />
            ) : (
              <JsonResponse content={updatedReservationbycode || error} />
            )}
          </Box>
        </Box>
      )}
