...
export default function SpecialRequestManagementUpdateSpecialRequestApiPage() {
  const [view, setView] = useState("Table");
  const [updatedSpecialrequest, setUpdatedSpecialrequest] = useState(null);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      setError(null);
      setUpdatedSpecialrequest(null);
      reset();
      console.info("RESPONSE", response);
      setUpdatedSpecialrequest(response.data.specialRequest);
    } catch (ex) {
      console.error(ex);
      setError(ex);
    }
    ...
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      {(updatedSpecialrequest || error) && (
        <Box paddingTop="2rem">
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse content={updatedSpecialrequest} error={error} />
            ) : (
              <JsonResponse content={updatedSpecialrequest || error} />
            )}
          </Box>
        </Box>
      )}
