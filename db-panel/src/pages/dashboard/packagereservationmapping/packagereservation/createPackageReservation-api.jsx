...
export default function PackageReservationMappingCreatePackageReservationApiPage() {
  const [view, setView] = useState("Table");
  const [createdPackagereservation, setCreatedPackagereservation] =
    useState(null);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      setError(null);
      setCreatedPackagereservation(null);
      reset();
      console.info("RESPONSE", response);
      setCreatedPackagereservation(response.data.packageReservation);
    } catch (ex) {
      console.error(ex);
      setError(ex);
    }
  ...
  return (
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      <Divider />
      {(createdPackagereservation || error) && (
        <Box paddingTop="2rem">
          <Box
            display="flex"
            flexDirection="row"
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse
                content={createdPackagereservation}
                error={error}
              />
            ) : (
              <JsonResponse content={createdPackagereservation || error} />
            )}
          </Box>
        </Box>
      )}
