...
export function NavUser({
  ...
  const renderAvatar = () =>
    user.avatar ? (
      <Avatar alt={user.nameToUse} src={user.avatar} />
    ) : (
      <Avatar alt={user.nameToUse}>
        {user.nameToUse.charAt(0).toUpperCase()}
      </Avatar>
    );
    ...
    >
      <NavUl sx={{ flex: "1 1 auto", gap: "var(--nav-item-gap)" }}>
        {/* <NavLi sx={slotProps}>
                    <NavItem
                      ref={navItemRef}
                      title="Panel Repository"
                      depth={1}
                      slotProps={slotProps?.rootItem}
                      href="https://gitlab.mindbricks.com/db/db-panel.git"
                      target="_blank"
                      icon={<Iconify icon="solar:code-file-broken" />}
                    />
                </NavLi> */}
