import {
  Chip,
  Dialog,
  DialogActions,
  DialogContent,
  DialogContentText,
  DialogTitle,
  Link,
  Paper,
  Table,
  TableBody,
  TableCell,
  TableContainer,
  TableRow,
} from "@mui/material";
import { TableHeadCustom } from "../../../table/index.js";
import { Iconify } from "../../../iconify/index.js";
import { useTheme } from "@mui/material/styles";

import { Form, Field } from "../../../hook-form";
import * as zod from "zod";
import { useState, useEffect } from "react";
import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import LoadingButton from "@mui/lab/LoadingButton";
import { mutate } from "swr";

import paymentManagementAxios, {
  paymentManagementEndpoints,
} from "../../../../lib/paymentManagement-axios.js";

const UPDATE_TABLE_HEAD = [
  { id: "propertyName", label: "Property Name", width: "30%" },
  { id: "propertyValue", label: "Property Value", width: "70%" },
];

const PaymentSchema = zod.object({
  reservationId: zod.string().nullable(),

  amount: zod.number().nullable(),

  paymentStatus: zod.string().nullable(),

  paymentMethod: zod.string().nullable(),

  gatewayReference: zod.string().nullable(),

  processedAt: zod.string().nullable(),
});

export default function ({ openDialog, selectedPayment }) {
  const [error, setError] = useState(null);

  const theme = useTheme();

  const requestParams = [
    {
      name: "reservationId",
      value: selectedPayment?.reservationId ?? "",
      type: "ID",
    },

    { name: "amount", value: selectedPayment?.amount ?? "", type: "Double" },

    {
      name: "paymentStatus",
      value: selectedPayment?.paymentStatus ?? "",
      type: "Enum",
    },

    {
      name: "paymentMethod",
      value: selectedPayment?.paymentMethod ?? "",
      type: "Enum",
    },

    {
      name: "gatewayReference",
      value: selectedPayment?.gatewayReference ?? "",
      type: "String",
    },

    {
      name: "processedAt",
      value: selectedPayment?.processedAt ?? "",
      type: "Date",
    },
  ];

  const defaultValues = {
    reservationId: selectedPayment?.reservationId ?? "",

    amount: selectedPayment?.amount ?? "",

    paymentStatus: selectedPayment?.paymentStatus ?? "",

    paymentMethod: selectedPayment?.paymentMethod ?? "",

    gatewayReference: selectedPayment?.gatewayReference ?? "",

    processedAt: selectedPayment?.processedAt ?? "",
  };

  const methods = useForm({
    resolver: zodResolver(PaymentSchema),
    defaultValues,
  });

  const {
    reset,
    watch,
    setValue,
    handleSubmit,
    formState: { isSubmitting },
  } = methods;

  const onSubmit = handleSubmit(async (data) => {
    try {
      if (selectedPayment?.id) {
        const response = await paymentManagementAxios.patch(
          paymentManagementEndpoints.payment.updatePayment.replace(
            ":paymentId",
            selectedPayment?.id,
          ),
          data,
        );
        setError(null);
        reset();
        console.info("RESPONSE", response);
        await mutate([paymentManagementEndpoints.payment.listPayments]);
        openDialog.onFalse();
      }
    } catch (ex) {
      console.error(ex);
      setError(ex);
    }
  });

  useEffect(() => {
    methods.reset({
      reservationId: selectedPayment?.reservationId ?? "",

      amount: selectedPayment?.amount ?? "",

      paymentStatus: selectedPayment?.paymentStatus ?? "",

      paymentMethod: selectedPayment?.paymentMethod ?? "",

      gatewayReference: selectedPayment?.gatewayReference ?? "",

      processedAt: selectedPayment?.processedAt ?? "",
    });
  }, [selectedPayment]);

  if (!selectedPayment) return null;

  return (
    <Dialog open={openDialog.value} maxWidth="md">
      <DialogTitle>Update Payment</DialogTitle>

      <Form methods={methods} onSubmit={onSubmit}>
        <DialogContent>
          <TableContainer component={Paper}>
            <Table sx={{ minWidth: 600 }}>
              <TableHeadCustom headCells={UPDATE_TABLE_HEAD} />

              <TableBody>
                {requestParams.map((row) => (
                  <TableRow key={row.name}>
                    <TableCell
                      sx={{ backgroundColor: theme.palette.grey[100] }}
                    >
                      <Chip variant="soft" label={row.name} />
                    </TableCell>
                    <TableCell>
                      {row.type === "Boolean" ? (
                        <Field.Checkbox name={row.name} />
                      ) : (
                        <Field.Text name={row.name} />
                      )}
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
          </TableContainer>
          {error && (
            <DialogContentText color="error">
              {error.message
                ? error.message
                : "An error occurred while creating the payment."}
            </DialogContentText>
          )}
        </DialogContent>

        <DialogActions className="gap-2">
          <Link
            component="button"
            type="button"
            underline="always"
            onClick={openDialog.onFalse}
          >
            Cancel
          </Link>
          <LoadingButton
            type="submit"
            variant="contained"
            size="large"
            loading={isSubmitting}
            startIcon={<Iconify icon="material-symbols:save-outline" />}
          >
            Save
          </LoadingButton>
        </DialogActions>
      </Form>
    </Dialog>
  );
}
