...
console.log("Starting db-roompricing-service service...");
...
const start = async () => {
  process.title = "node-db-roompricing-service";
  ...
  try {
    ...
    await setRedisData("db-roompricing-service:start", today.toUTCString());
    console.log(
      "Service start datetime is written to Redis key db-roompricing-service:start",
      today.toUTCString(),
    );
  } catch (err) {
  ...
  console.log(
    "db-roompricing-service is listening HTTP/REST port " +
...
/* starting the server db-roompricing-service .... */
start();
