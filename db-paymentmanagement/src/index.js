...
console.log("Starting db-paymentmanagement-service service...");
...
const start = async () => {
  process.title = "node-db-paymentmanagement-service";
  ...
  try {
    ...
    await setRedisData(
      "db-paymentmanagement-service:start",
      today.toUTCString(),
    );
    console.log(
      "Service start datetime is written to Redis key db-paymentmanagement-service:start",
      today.toUTCString(),
    );
  } catch (err) {
  ...
  console.log(
    "db-paymentmanagement-service is listening HTTP/REST port " +
...
/* starting the server db-paymentmanagement-service .... */
start();
