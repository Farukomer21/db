...
_Resource Definition_ : Hotel reservation record linking guest, room, payment, packages, special requests, and a secure reservationCode for guest access. Used for both staff and guest workflows.
_Reservation Resource Properties_
| Name | Type | Required | Default | Definition |
| ---- | ---- | -------- | ------- | ---------- |
| **guestId** | ID | | | _Reference to guest who made this reservation._ |
| **roomId** | ID | | | _Reference to the hotel room for this reservation._ |
| **checkInDate** | Date | | | _Check-in date for the reservation._ |
| **checkOutDate** | Date | | | _Check-out date for the reservation._ |
| **reservationCode** | String | | | _Unique, cryptographically secure reservation code for guest access. Null until payment is confirmed._ |
| **packages** | ID | | | _Array of package IDs associated to this reservation (maps to packageReservationMapping)._ |
| **specialRequests** | ID | | | _Array of specialRequest IDs associated with this reservation._ |
| **paymentId** | ID | | | _Reference to the payment record for this reservation._ |
| **status** | Enum | | | _Reservation status: pending=not yet paid, confirmed=paid, canceled, completed=after stay._ |
| **numGuests** | Integer | | | _Number of guests for this reservation._ |
| **totalPrice** | Double | | | _Total price for the reservation (rooms + packages)._ |
| **notes** | Text | | | _Internal notes, only for staff eyes (not sent to guests/public)._ |
...
_API Definition_ : Staff API to create new reservation record with any status. Internal use only.
...
The createReservation api has got 11 parameters
...
_API Definition_ : Staff API to update any reservation field. Internal use only.
...
_API Definition_ : Staff API to get reservation by ID. Internal use only.
...
> For a detailed description of the `` api with its internal and inter-service logic please refer to the [Business API Specification Document For Get Reservation](businessApi/getReservation).

### Delete Reservation API

_API Definition_ : Staff API to delete reservation by ID (soft-delete enabled). Internal use only.
...
_API Definition_ : Staff API to list reservations for hotel operations. Internal use only.
...
_API Definition_ : Guest-facing API. Create a reservation in &#39;pending&#39; status (pre-payment, no code yet), with guest, dates, room, packages, and optional special requests.

_API Crud Type_ : create

_Default access route_ : _POST_ `/v1/reservationguest`

#### Parameters

The createReservationGuest api has got 11 parameters
...
> For a detailed description of the `` api with its internal and inter-service logic please refer to the [Business API Specification Document For Create Reservationguest](businessApi/createReservationGuest).

### Confirm Reservationpayment API

_API Definition_ : Guest API. After external payment is successful, mark reservation as confirmed, generate &amp; store reservationCode.

_API Crud Type_ : update

_Default access route_ : _PATCH_ `/v1/confirmreservationpayment/:reservationId`

#### Parameters

The confirmReservationPayment api has got 12 parameters
...
> For a detailed description of the `` api with its internal and inter-service logic please refer to the [Business API Specification Document For Confirm Reservationpayment](businessApi/confirmReservationPayment).

### Get Reservationbycode API

_API Definition_ : Guest self-service. Retrieve reservation by reservationCode (no auth).
...
_API Definition_ : Guest self-service. Update reservation by reservationCode. Only if status allows; restrict to editable fields for guest.

_API Crud Type_ : update

_Default access route_ : _PATCH_ `/v1/reservationbycode/:reservationId`

#### Parameters

The updateReservationByCode api has got 12 parameters
...
> For a detailed description of the `` api with its internal and inter-service logic please refer to the [Business API Specification Document For Update Reservationbycode](businessApi/updateReservationByCode).

### Cancel Reservationbycode API

_API Definition_ : Guest self-service. Cancel reservation using reservationCode. Only possible if not already canceled or completed.
