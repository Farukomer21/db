...
const { dbScriptConfirmReservationpayment } = require("dbLayer");

class ConfirmReservationPaymentManager extends ReservationManager {
  ...
  async doBusiness() {
    const reservationpayment = await dbScriptConfirmReservationpayment(this);
  ...
  async raiseEvent() {
    ReservationpaymentConfirmedPublisher.Publish(
      this.output,
      this.session,
    ).catch((err) => {
...
module.exports = ConfirmReservationPaymentManager;
