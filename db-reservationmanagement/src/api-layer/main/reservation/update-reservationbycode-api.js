...
const { dbScriptUpdateReservationbycode } = require("dbLayer");

class UpdateReservationByCodeManager extends ReservationManager {
  ...
  async buildDataClause() {
    ...
    const dataClause = {
      ...
      packages: this.packages
        ? this.packages
        : this.packages_remove
          ? sequelize.fn(
              "array_remove",
              sequelize.col("packages"),
              this.packages_remove,
            )
          : this.packages_append
            ? sequelize.fn(
                "array_append",
                sequelize.col("packages"),
                this.packages_append,
      ...
      specialRequests: this.specialRequests
        ...
        : this.specialRequests_remove
          ? sequelize.fn(
              "array_remove",
              sequelize.col("specialRequests"),
              this.specialRequests_remove,
            )
          : this.specialRequests_append
            ? sequelize.fn(
                "array_append",
                sequelize.col("specialRequests"),
                this.specialRequests_append,
  ...
  async doBusiness() {
    const reservationbycode = await dbScriptUpdateReservationbycode(this);
  ...
  async raiseEvent() {
    ReservationbycodeUpdatedPublisher.Publish(this.output, this.session).catch(
      (err) => {
...
module.exports = UpdateReservationByCodeManager;
