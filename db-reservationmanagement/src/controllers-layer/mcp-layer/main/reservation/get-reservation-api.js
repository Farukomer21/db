...
class GetReservationMcpController extends ReservationManagementMcpController {
  constructor(params) {
    super("getReservation", "getreservation", params);
    this.dataName = "reservation";
    this.crudType = "get";
  }

  createApiManager() {
    return new GetReservationManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    name: "getReservation",
    description: "Staff API to get reservation by ID. Internal use only.",
    parameters: GetReservationMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new GetReservationMcpController(mcpParams);
      try {
        const result = await controller.processRequest();
        //return GetReservationMcpController.getOutputSchema().parse(result);
