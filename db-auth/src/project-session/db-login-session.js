...
class DbLoginSession extends DbSession {
  ...
  async relogin(req) {
    console.log("db session found but a relogin is requested");
    try {
      const userField = "id";
  ...
  async setLoginToRequest(req, byPassword, bySubject) {
    ...
    session.checkTokenMark = "db-inapp-token";
    session._USERID = session.userId;
    ...
    const cookieName = `db-access-token`;
  ...
  async initSuperAdmin() {
    try {
      const absUserData = {
        ...
        fullname: "Root User",
        email: "omerfarukaydon@gmail.com",
        emailVerified: true,

        password: hashString("Mindbricks321"),
