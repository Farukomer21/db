...
console.log("Starting db-auth-service service...");
...
const DbLoginSession = require("./project-session/db-login-session");

let processActive = false;
let expressServer = null;

const start = async () => {
  process.title = "node-db-auth-service";
  ...
  try {
    ...
    await setRedisData("db-auth-service:start", today.toUTCString());
    console.log(
      "Service start datetime is written to Redis key db-auth-service:start",
      today.toUTCString(),
    );
  } catch (err) {
  ...
  console.log(
    "db-auth-service is listening HTTP/REST port " + servicePort.toString(),
  );
  console.log(
  ...
  // init login database
  const loginSession = new DbLoginSession();
...
/* starting the server db-auth-service .... */
start();
