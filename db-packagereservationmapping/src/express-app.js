...
const exposedHeaders = ["db-access-token", "db-refresh-token", "set-cookie"];
if (!corsOrigin) {
  app.use(
    cors({
...
const {
  ...
  packageReservationRouter,
} = require("restLayer");

app.use("", packageReservationRouter);
...
app.get("/getPostmanCollection", (req, res) => {
  const filePath = path.join(
    __dirname,
    "db-packageReservationMapping-postman-collection.json",
  );
  console.log("Downloading Postman Collection from:", filePath);
  res.setHeader("Content-Type", "application/json");
  res.setHeader(
    "Content-Disposition",
    'attachment; filename="db-packageReservationMapping-postman-collection.json"',
  );
