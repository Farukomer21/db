...
const getPackageReservationStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(PackageReservation.count({ where: queryWithSoftDelete }));
        statLabels.push("count");
      } else if (statParts.length == 2) {
        if (statParts[0] === "sum") {
          promises.push(
            PackageReservation.sum(statParts[1], {
          ...
          promises.push(
            PackageReservation.avg(statParts[1], {
              where: queryWithSoftDelete,
            }),
          );
          ...
          promises.push(
            PackageReservation.min(statParts[1], {
              where: queryWithSoftDelete,
            }),
          );
          ...
          promises.push(
            PackageReservation.max(statParts[1], {
              where: queryWithSoftDelete,
            }),
          );
    ...
    if (promises.length == 0) {
      return await PackageReservation.count({ where: queryWithSoftDelete });
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingPackageReservationStatsByQuery",
      err,
    );
  }
};

module.exports = getPackageReservationStatsByQuery;
