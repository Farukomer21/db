...
// This script is called from the DeletePackageReservationManager
...
const dbScriptDeletePackagereservation = async (apiManager) => {
  const whereClause = apiManager.whereClause;

  try {
    let rowsCount = null;
    let dbDoc = null;
    [rowsCount, [dbDoc]] = await PackageReservation.update(
      { isActive: false },
      {
        where: whereClause,
        returning: true,
    ...
    apiManagerpackageReservation = dbData;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptDeletePackagereservation",
      {
...
module.exports = dbScriptDeletePackagereservation;
