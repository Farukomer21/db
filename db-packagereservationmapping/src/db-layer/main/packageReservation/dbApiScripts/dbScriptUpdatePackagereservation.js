...
// This script is called from the UpdatePackageReservationManager
...
async function getEventPayload(apiManager, dataClause) {
  ...
  const newData = apiManager.packageReservation;
  return {
    old_packageReservation: oldData,
    packageReservation: newData,
...
const dbScriptUpdatePackagereservation = async (apiManager) => {
  ...
  try {
    ...
    let rowsCount = null;
    let dbDoc = null;
    [rowsCount, [dbDoc]] = await PackageReservation.update(dataClause, {
      where: whereClause,
      returning: true,
    });
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptUpdatePackagereservation",
      {
...
module.exports = dbScriptUpdatePackagereservation;
