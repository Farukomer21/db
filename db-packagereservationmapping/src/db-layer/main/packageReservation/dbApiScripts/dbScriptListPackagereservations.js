...
// This script is called from the ListPackageReservationsManager
...
const dbScriptListPackagereservations = async (apiManager) => {
  ...
  try {
    ...
    let packageReservationList = await PackageReservation.findAll(options);

    const dbData = { items: [] };
    apiManager.packageReservations = [];
    if (!packageReservationList || packageReservationList.length == 0)
      return dbData;

    dbData.items = Array.isArray(packageReservationList)
    ...
    apiManager.packageReservations = dbData.items;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptListPackagereservations",
      {
...
module.exports = dbScriptListPackagereservations;
