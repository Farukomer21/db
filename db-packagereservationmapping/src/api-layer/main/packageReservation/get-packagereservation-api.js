...
const { dbScriptGetPackagereservation } = require("dbLayer");

class GetPackageReservationManager extends PackageReservationManager {
  ...
  async doBusiness() {
    const packagereservation = await dbScriptGetPackagereservation(this);
  ...
  async raiseEvent() {
    PackagereservationRetrivedPublisher.Publish(
      this.output,
      this.session,
    ).catch((err) => {
...
module.exports = GetPackageReservationManager;
