...
_Resource Definition_ : Records special guest requests for hotel reservations and their staff-handling lifecycle. Linkable both to reservation and guest, with status for workflow control.
_SpecialRequest Resource Properties_
| Name | Type | Required | Default | Definition |
| ---- | ---- | -------- | ------- | ---------- |
| **reservationId** | ID | | | _Reference to the reservation for this special request._ |
| **guestId** | ID | | | _ID of the guest who made this special request (redundant for tracing, resolved via reservation also)._ |
| **requestText** | String | | | _Text description of the guest&#39;s special request._ |
| **status** | Enum | | | _Current status of the special request (&#39;pending&#39;,&#39;fulfilled&#39;,&#39;canceled&#39;)._ |
| **response** | String | | | _Staff action or response to the special request (free text)._ |
...
_API Definition_ : Staff API to create a special request for a given reservation and guest.
...
_API Definition_ : Staff API to update a special request by ID. Can edit status, response, or requestText.
...
_API Definition_ : Staff API to retrieve a special request by its ID.
...
_API Definition_ : Staff API to delete (soft delete) a special request by ID.
...
_API Definition_ : Staff API to list all special requests, with filter by reservationId, guestId, status. Internal use for fulfillment and monitoring.
...
_API Definition_ : Guest self-service: create a special request associated with their reservation (by reservationCode).

_API Crud Type_ : create

_Default access route_ : _POST_ `/v1/specialrequestpublic`
...
_API Definition_ : Guest public/self-service: list all their special requests, filtered by reservationCode (returned after payment/booking).

_API Crud Type_ : list

_Default access route_ : _GET_ `/v1/specialrequestsbycode`
...
> For a detailed description of the `` api with its internal and inter-service logic please refer to the [Business API Specification Document For List Specialrequestsbycode](businessApi/listSpecialRequestsByCode).

### Get Specialrequestbycode API

_API Definition_ : Guest public/self-service: retrieve a specific special request for their reservation (by reservationCode and specialRequestId).

_API Crud Type_ : get

_Default access route_ : _GET_ `/v1/specialrequestbycode/:specialRequestId`
...
_API Definition_ : Guest public/self-service: cancel their own (pending, unfulfilled) special request using reservationCode. Only possible by matching code and if status is &#39;pending&#39;.

_API Crud Type_ : update

_Default access route_ : _PATCH_ `/v1/cancelspecialrequestbycode/:specialRequestId`
...
> For a detailed description of the `` api with its internal and inter-service logic please refer to the [Business API Specification Document For Cancel Specialrequestbycode](businessApi/cancelSpecialRequestByCode).
