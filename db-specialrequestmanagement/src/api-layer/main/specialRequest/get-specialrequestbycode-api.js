...
const { dbScriptGetSpecialrequestbycode } = require("dbLayer");

class GetSpecialRequestByCodeManager extends SpecialRequestManager {
  ...
  async doBusiness() {
    const specialrequestbycode = await dbScriptGetSpecialrequestbycode(this);
  ...
  async raiseEvent() {
    SpecialrequestbycodeRetrivedPublisher.Publish(
      this.output,
      this.session,
    ).catch((err) => {
...
module.exports = GetSpecialRequestByCodeManager;
