...
console.log("Starting db-guestmanagement-service service...");
...
const start = async () => {
  process.title = "node-db-guestmanagement-service";
  ...
  try {
    ...
    await setRedisData("db-guestmanagement-service:start", today.toUTCString());
    console.log(
      "Service start datetime is written to Redis key db-guestmanagement-service:start",
      today.toUTCString(),
    );
  } catch (err) {
  ...
  console.log(
    "db-guestmanagement-service is listening HTTP/REST port " +
...
/* starting the server db-guestmanagement-service .... */
start();
