...
console.log("Starting db-personnelmanagement-service service...");
...
const start = async () => {
  process.title = "node-db-personnelmanagement-service";
  ...
  try {
    ...
    await setRedisData(
      "db-personnelmanagement-service:start",
      today.toUTCString(),
    );
    console.log(
      "Service start datetime is written to Redis key db-personnelmanagement-service:start",
      today.toUTCString(),
    );
  } catch (err) {
  ...
  console.log(
    "db-personnelmanagement-service is listening HTTP/REST port " +
...
/* starting the server db-personnelmanagement-service .... */
start();
