...
const consumer = kafka.consumer({
  groupId: `db-notification-service-userVerification-group`,
});

const userVerificationListener = async () => {
  await consumer.connect();
  await consumer.subscribe({
    topic: "db-user-service-email-verification-start",
    fromBeginning: true,
  });
  ...
  await consumer.run({
    eachMessage: async ({ topic, partition, message }) => {
      try {
        ...
        //**errorLog
        console.error("db-user-service-email-verification-start ", error);
      }
    },
  });
};
