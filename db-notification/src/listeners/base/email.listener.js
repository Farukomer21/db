...
const consumer = kafka.consumer({
  groupId: `db-notification-service-email-consumer`,
});
...
const runEmailSenderListener = async () => {
  ...
  await consumer.subscribe({
    topic: `db-notification-service-notification-email`,
    fromBeginning: true,
  });
  ...
  await consumer.run({
    eachMessage: async ({ topic, partition, message }) => {
      try {
        ...
        //**errorLog
        console.error(`db-notification-service-notification-email: `, error);
      }
    },
  });
};
