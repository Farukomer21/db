...
const consumer = kafka.consumer({
  groupId: `db-notification-service-sms-consumer`,
});
...
const runSmsSenderListener = async () => {
  ...
  await consumer.subscribe({
    topic: `db-notification-service-notification-sms`,
    fromBeginning: true,
  });
  ...
  await consumer.run({
    eachMessage: async ({ topic, partition, message }) => {
      try {
        ...
        //**errorLog
        console.error(`db-notification-service-notification-sms: `, error);
      }
    },
  });
};
