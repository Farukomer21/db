...
const consumer = kafka.consumer({
  groupId: `db-notification-service-push-consumer`,
});
...
const runPushSenderListener = async () => {
  ...
  await consumer.subscribe({
    topic: `db-notification-service-notification-push`,
    fromBeginning: true,
  });
  ...
  await consumer.run({
    eachMessage: async ({ topic, partition, message }) => {
      try {
        ...
        //**errorLog
        console.error(`db-notification-service-notification-push: `, error);
      }
    },
  });
};
